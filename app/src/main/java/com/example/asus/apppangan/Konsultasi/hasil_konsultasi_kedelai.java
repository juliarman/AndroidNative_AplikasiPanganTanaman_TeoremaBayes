...
public class hasil_konsultasi_kedelai extends AppCompatActivity  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       int i;
        ...
        setContentView((int) R.layout.activity_hasil_konsultasi_kedelai);
        ...
        this.cursor1 = db.rawQuery("SELECT p.namapenyakit from rulekedelai r inner join penyakitkedelai p on r.id_penyakit = p.id_penyakit inner join gejalakedelai g on r.id_gejala = g.id_gejala where g.namagejala in (" + getIntent().getStringExtra("namagejala") + ") group by p.namapenyakit order by namapenyakit ASC", null);
        ...
        for (i = 0; i < this.cursor1.getCount(); i++) {
            ...
            this.cursor2 = db.rawQuery("SELECT r.probabilitas from rulekedelai r inner join penyakitkedelai p on r.id_penyakit  = p.id_penyakit inner join gejalakedelai g on r.id_gejala = g.id_gejala where g.namagejala  in(" + getIntent().getStringExtra("namagejala") + ") AND p.namapenyakit = '" + this.penyakit_terpilih[i] + "'", null);
            this.cursor3 = db.rawQuery("SELECT  p.probabilitas from penyakitkedelai p inner join rulekedelai r on r.id_penyakit  = p.id_penyakit inner join gejalakedelai g on r.id_gejala = g.id_gejala where g.namagejala  in(" + getIntent().getStringExtra("namagejala") + ") AND p.namapenyakit = '" + this.penyakit_terpilih[i] + "'", null);
   ...
   /* public void prosesakedelai(View view) {
        String nama_penyakit = ((TextView) view.findViewById(R.id.textView3)).getText().toString();
        Intent i = new Intent(this, detail_penyakit_kedelai.class);
